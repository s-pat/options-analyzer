  ...
  if (isLoading) {
    return (
      <tr className="border-b border-white/[0.04] animate-pulse">
        {[14, 36, 20, 12, 28, 10].map((w, i) => (
          <td key={i} className="px-4 py-3"><div className={`shimmer h-4 w-${w} rounded`} /></td>
        ))}
      </tr>
    );
  }
    ...
    return (
      <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-4 animate-pulse space-y-2">
        <div className="flex justify-between">
          <div className="shimmer h-4 w-14 rounded" /><div className="shimmer h-4 w-20 rounded" />
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-white/[0.06]">
                {['Symbol', 'Company', 'Earnings Date', 'Days Until', 'EPS Est.', 'Risk'].map((h, i) => (
                  <th key={h} className={cn(
                    'px-4 py-3 text-[10px] font-medium uppercase tracking-wide text-white/30',
